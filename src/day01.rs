...
fn part1(input: &[u32]) -> u32 {
    input.iter().copied().map(mass2fuel).sum()
}

#[aoc(day1, part2, recursive)]
fn part2_recursive(input: &[u32]) -> u32 {
    input.iter().copied().map(mass2fuel_recursive).sum()
}

#[aoc(day1, part2, iterative)]
fn part2_iterative(input: &[u32]) -> u32 {
    input.iter().copied().map(mass2fuel_iterative).sum()
}

fn mass2fuel(mass: u32) -> u32 {
    (mass / 3).saturating_sub(2)
}

fn mass2fuel_recursive(mass: u32) -> u32 {
    let mut fuel = (mass / 3).saturating_sub(2);
    if fuel > 0 {
        fuel += mass2fuel_recursive(fuel);
    }
    fuel
}

fn mass2fuel_iterative(mass: u32) -> u32 {
    let mut res = 0;
    let mut delta = mass2fuel(mass);
    while delta > 0 {
        res += delta;
        delta = mass2fuel(delta);
    }
    res
}
...
#[test]
fn test_mass2fuel_recursive() {
    assert_eq!(2, mass2fuel_recursive(12));
    assert_eq!(966, mass2fuel_recursive(1969));
    assert_eq!(50346, mass2fuel_recursive(100756));
}

#[test]
fn test_mass2fuel_iterative() {
    assert_eq!(2, mass2fuel_iterative(12));
    assert_eq!(966, mass2fuel_iterative(1969));
    assert_eq!(50346, mass2fuel_iterative(100756));
}
