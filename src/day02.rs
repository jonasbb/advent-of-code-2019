...
impl Program {
    ...
    {
        // Load the three args
        let out = self.data[self.program_counter + 3];
        let a = self.data[self.program_counter + 1];
        let b = self.data[self.program_counter + 2];
        // Deref the input arguments
        let a = self.data[a];
        let b = self.data[b];
        self.data[out] = f(a, b);
    }

    fn step(&mut self) -> ProgramExecutionState {
        match self.data[self.program_counter] {
            99 => return ProgramExecutionState::Finished,
            1 => self.execute(|a, b| a + b),
            2 => self.execute(|a, b| a * b),
            x => unreachable!("Unknown OP code {}", x),
        };
    ...
    fn run(&mut self) -> usize {
        while self.step() == ProgramExecutionState::Running {}
        self.data[0]
    }
}
...
fn input_generator(input: &str) -> Program {
    Program {
        data: input.split(',').map(|x| x.parse().unwrap()).collect(),
        program_counter: 0,
    }
}
...
#[test]
fn part1_program_step() {
    let mut prog = Program {
        data: vec![1, 0, 0, 0, 99],
        program_counter: 0,
    };
    assert_eq!(ProgramExecutionState::Running, prog.step());
    assert_eq!(
        Program {
            data: vec![2, 0, 0, 0, 99],
            program_counter: 4,
    ...
    assert_eq!(ProgramExecutionState::Finished, prog.step());
    assert_eq!(
        Program {
            data: vec![2, 0, 0, 0, 99],
            program_counter: 4,
...
#[test]
fn part1_program_run1() {
    let mut prog = Program {
        data: vec![1, 0, 0, 0, 99],
        program_counter: 0,
    };
    assert_eq!(2, prog.run());
    assert_eq!(
        Program {
            data: vec![2, 0, 0, 0, 99],
            program_counter: 4,
...
#[test]
fn part1_program_run4() {
    let mut prog = Program {
        data: vec![1, 1, 1, 4, 99, 5, 6, 0, 99],
        program_counter: 0,
    };
    assert_eq!(30, prog.run());
    assert_eq!(
        Program {
            data: vec![30, 1, 1, 4, 2, 5, 6, 0, 99],
            program_counter: 8,
        },
        prog
    );
